...
// Nội dung "phiếu in" cho 1 lệnh chuyển cấp chất lượng — dùng chung cho cả trang Xử lý lệnh (in
// ngay tại chỗ, dữ liệu đã có sẵn) và trang Chuyển cấp chất lượng (danh sách, in nhanh từ nút trên
// bảng, không cần điều hướng sang trang chi tiết). Giống hệt cơ chế đã làm cho LenhPrintView (Nhập/
// Xuất) — ẩn khi xem trên màn hình, chỉ hiện khi in (xem .print-only trong HoSoTbDongBo.css).
export default function ChuyenCapPrintView({ lenh }) {
  if (!lenh) return null;
  ...
  return (
    <div className="print-only">
      ...
      <div className="print-title">Lệnh chuyển cấp chất lượng</div>
      ...
      <div style={{ fontWeight: 200, fontSize: 18, marginBottom: 6 }}>Danh sách TB chuyển cấp</div>
        ...
        <tbody>
          {danhSach.map((r, i) => (
            <tr key={r.maCtLenhChuyenCap}>
              ...
              <td style={{ textAlign: 'center' }}>{r.capCu}</td>
              <td style={{ textAlign: 'center' }}>{r.soLuong}</td>
              <td style={{ textAlign: 'center' }}>{r.capMoi}</td>
              <td>{r.ghiChu || ''}</td>
            </tr>
          ))}
      ...
      <div className="print-signatures">
        <div>
          ...
          <div className="print-signature-title">Cấp trên</div>
        </div>
        <div>
