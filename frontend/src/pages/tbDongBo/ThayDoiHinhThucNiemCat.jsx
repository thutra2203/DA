...
export default function ThayDoiHinhThucNiemCat() {
  ...
  // chi tiết, thayDoiHtncAPI.getOne trả về sẵn cả 2 trong 1 lần gọi) rồi mở hộp thoại in ngay khi có
  // đủ dữ liệu.
  const handlePrint = async () => {
    if (!maLenhChonIn) return;
    setDangInLenh(true);
    try {
      const res = await thayDoiHtncAPI.getOne(maLenhChonIn);
  ...
  return (
    <div>
      <div className="data-card">
        ...
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <div className="empty-state-desc">Lệnh do cấp trên tạo sẽ hiển thị ở đây để thực hiện</div>
          </div>
        ) : (
                  ...
                  <tr key={r.maLenh}>
                    ...
                    <td className="td-center">
                      <div className="td-actions">
                        ...
                        <button className="btn-icon-warn" onClick={() => navigate(`/tb-dong-bo/thay-doi-htnc/${r.maLenh}`)} title="Xử lý">
                          <FiEdit2 size={13} />
                        </button>
                      </div>
                    </td>
      ...
      <ThayDoiHtncPrintView lenh={printData} />
...
// Xem thông tin các trường của bản ghi lệnh (bảng LenhThayDoiHtnc) — không phải dòng chi tiết (đã
// có ở trang Xử lý) và không phải bản in trang trọng (đã có ở nút "In lệnh"). Dữ liệu lấy thẳng từ
