...
export default function TaoLenhThayDoiViTri() {
  usePageTitle('Tạo lệnh thay đổi vị trí');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing) {
        await thayDoiViTriAPI.suaLenh(editing.maLenh, payload);
        showToast('Cập nhật thành công!');
      } else {
        await thayDoiViTriAPI.taoLenh(payload);
        showToast('Tạo lệnh thành công! Vào "Thay đổi vị trí" để thêm dòng chi tiết.');
      }
      setShowModal(false);
      loadItems();
  ...
  const handleDelete = async (row) => {
    if (!(await confirm(`Xóa lệnh thay đổi vị trí "${row.maLenh}"? Toàn bộ dòng chi tiết trong lệnh sẽ bị xóa theo.`))) return;
    try {
      await thayDoiViTriAPI.xoaLenh(row.maLenh);
      ...
      <div className="data-card">
        <div className="table-toolbar">
          ...
          <button className="btn-add" onClick={openAdd}>
            <FiPlus style={{ marginRight: 6 }} />Tạo lệnh thay đổi vị trí
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <div className="empty-state-desc">Nhấn "Tạo lệnh thay đổi vị trí" để bắt đầu</div>
          </div>
        ) : (
        ...
        <div className="overlay">
          <div className="modal modal--form fade-in">
            <div className="modal-header">
              <h3 className="modal-title">{editing ? 'Sửa lệnh thay đổi vị trí' : 'Tạo lệnh thay đổi vị trí'}</h3>
...
// (đã có ở trang "Thay đổi vị trí") và không phải bản in trang trọng.
