...
// Nội dung "phiếu in" cho 1 lệnh thay đổi hình thức niêm cất — dùng chung cho cả trang Xử lý lệnh
// (in ngay tại chỗ, dữ liệu đã có sẵn) và trang Thay đổi hình thức niêm cất (danh sách, in nhanh từ
// nút trên bảng, không cần điều hướng sang trang chi tiết). Giống hệt cơ chế đã làm cho
// ChuyenCapPrintView.
export default function ThayDoiHtncPrintView({ lenh }) {
  if (!lenh) return null;
  ...
  return (
    <div className="print-only">
      ...
      <div className="print-title">Lệnh thay đổi hình thức niêm cất</div>
      ...
      <div style={{ fontWeight: 200, fontSize: 18, marginBottom: 6 }}>Danh sách TB thay đổi hình thức niêm cất</div>
        ...
        <tbody>
          {danhSach.map((r, i) => (
            <tr key={r.maCtLenhThayDoiHtnc}>
              ...
              <td style={{ textAlign: 'center' }}>{r.htncCu || ''}</td>
              <td style={{ textAlign: 'center' }}>{r.soLuong}</td>
              <td style={{ textAlign: 'center' }}>{r.htncMoi}</td>
              <td>{r.ghiChu || ''}</td>
            </tr>
          ))}
