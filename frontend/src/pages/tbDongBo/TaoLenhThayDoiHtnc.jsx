...
export default function TaoLenhThayDoiHtnc() {
  usePageTitle('Tạo lệnh thay đổi hình thức niêm cất');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing) {
        await thayDoiHtncAPI.suaLenh(editing.maLenh, payload);
        showToast('Cập nhật thành công!');
      } else {
        await thayDoiHtncAPI.taoLenh(payload);
        showToast('Tạo lệnh thành công! Vào "Thay đổi hình thức niêm cất" để thêm dòng chi tiết.');
      }
      setShowModal(false);
      loadItems();
  ...
  const handleDelete = async (row) => {
    if (!(await confirm(`Xóa lệnh thay đổi hình thức niêm cất "${row.maLenh}"? Toàn bộ dòng chi tiết trong lệnh sẽ bị xóa theo.`))) return;
    try {
      await thayDoiHtncAPI.xoaLenh(row.maLenh);
    ...
    <div>
      ...
      <div className="data-card">
        <div className="table-toolbar">
          <span className="table-total">Tổng: <strong>{bySearch.length}</strong> lệnh thay đổi hình thức niêm cất</span>
        ...
        {loading ? (
          ...
          <div className="empty-state">
            <div className="empty-state-title">Chưa có lệnh thay đổi hình thức niêm cất nào</div>
            <div className="empty-state-desc">Nhấn "Tạo lệnh thay đổi hình thức niêm cất" để bắt đầu</div>
          </div>
        ) : (
        ...
        <div className="overlay">
          <div className="modal modal--form fade-in">
            <div className="modal-header">
              <h3 className="modal-title">{editing ? 'Sửa lệnh thay đổi hình thức niêm cất' : 'Tạo lệnh thay đổi hình thức niêm cất'}</h3>
...
// Xem thông tin các trường của bản ghi lệnh (bảng LenhThayDoiHtnc) — không phải dòng chi tiết
// (đã có ở trang "Thay đổi hình thức niêm cất") và không phải bản in trang trọng.
function ChiTietLenhModal({ row, khoMap, onClose }) {
  const rows = [
    ...
    ['Trạng thái', row.daKetThuc ? 'Đã kết thúc' : 'Đang soạn'],
    ['Số dòng', row.soDong ?? ''],
    ['Căn cứ', row.canCu || ''],
    ['Về việc', row.veViec || ''],
    ['Ghi chú', row.ghiChu || ''],
