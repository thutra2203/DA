...
import BaoCaoDongBoPhaoPrintView from './BaoCaoDongBoPhaoPrintView';
...
const META_MAC_DINH = {
  ...
  tieuDeChinh: 'BÁO CÁO TÌNH HÌNH ĐỒNG BỘ CÁC LOẠI PHÁO', tieuDePhu: '',
  noiNhan: '',
...
// Form "Thông tin báo cáo" để tạo Báo cáo tình hình đồng bộ các loại pháo — cùng cấu trúc với
// TaoBaoCaoDongBoModal (súng bộ binh), chỉ khác nguồn số liệu (baoCaoAPI.getDongBoPhao, gộp 4 Nhóm
// SPKT "pháo") và print view (BaoCaoDongBoPhaoPrintView, header 3 cấp do có nhóm cột cha).
export default function TaoBaoCaoDongBoPhaoModal({ onClose }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      if (hanhDongCho === 'print') {
        ...
        try {
          await taiPrintViewThanhPdf(`bao-cao-dong-bo-phao-${selectedKho}.pdf`);
        } catch {
  ...
  const layDuLieuVaThucHien = async (hanhDong) => {
    ...
    try {
      const res = await baoCaoAPI.getDongBoPhao(selectedKho);
  ...
  return (
    <div className="overlay" onClick={onClose}>
      ...
      <BaoCaoDongBoPhaoPrintView data={printData} meta={meta} />
    </div>
  );
}
