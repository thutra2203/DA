...
import BaoCaoNhuCauDongBoPrintView from './BaoCaoNhuCauDongBoPrintView';
...
const META_MAC_DINH = {
  ...
  tieuDeChinh: 'BÁO CÁO NHU CẦU ĐỒNG BỘ SÚNG PHÁO KHÍ TÀI', tieuDePhu: 'Mẫu số: 34/18/QK-VK',
  noiNhan: '',
...
// Form "Thông tin báo cáo" để tạo Báo cáo nhu cầu đồng bộ súng pháo khí tài (Mẫu 34/18/QK-VK) —
// cùng cấu trúc form với 2 báo cáo đồng bộ kia, chỉ khác nguồn số liệu (baoCaoAPI.getNhuCauDongBo,
// gộp TẤT CẢ Nhóm SPKT chứ không giới hạn 1 nhóm) và print view (BaoCaoNhuCauDongBoPrintView, phụ
// tùng liệt kê theo dòng thay vì cột cố định).
export default function TaoBaoCaoNhuCauDongBoModal({ onClose }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      if (hanhDongCho === 'print') {
        ...
        try {
          await taiPrintViewThanhPdf(`bao-cao-nhu-cau-dong-bo-${selectedKho}.pdf`);
        } catch {
  ...
  const layDuLieuVaThucHien = async (hanhDong) => {
    ...
    try {
      const res = await baoCaoAPI.getNhuCauDongBo(selectedKho);
      setHanhDongCho(hanhDong);
      setPrintData(res.data);
    } catch {
      setLoi('Không tải được số liệu nhu cầu đồng bộ');
      setDangTao(false);
      setDangTaiPdf(false);
    }
  };
  ...
  return (
    <div className="overlay" onClick={onClose}>
      ...
      <BaoCaoNhuCauDongBoPrintView data={printData} meta={meta} />
    </div>
  );
}
